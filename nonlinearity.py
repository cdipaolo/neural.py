...
class Softmax():
	...
	>>> Softmax.f(np.array([[10, -10, 13],[-98, 106, 140]]))
	array([[  4.74258732e-002,   9.77520103e-011,   9.52574127e-001],
	       [  4.34423497e-104,   1.71390843e-015,   1.00000000e+000]])
        '''
        x = x.astype('float64')
        shape = x.shape
        if len(x.shape) == 1:
...
class LogSoftmax():
    ...
    def f(x):
        ...
        >>> LogSoftmax.f(np.array([[-999,1,1],[1,-999,1],[1,1,-999],[1,1,1]]))
	array([[ -1.00069315e+03,  -6.93147181e-01,  -6.93147181e-01],
